...
class Board
{
   ...
   {	
      ...
      int upCheck = check(r-1,c);
      int downCheck = check(r+1,c);
      int leftCheck = check(r,c-1);
      int rightCheck = check(r,c+1); 
      return 1 + Math.min(upCheck,Math.min(downCheck,Math.min(leftCheck,rightCheck)));
   }	
