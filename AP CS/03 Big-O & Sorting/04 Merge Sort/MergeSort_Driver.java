...
class MergeSort
{
   ...
   */
                            ...
                            int low, int middle, int high)
   {
      ...
      // We need these indexes in order to be able to keep track of the pointer while sorting
      int indexLeft = low;
      ...
      for(int i = low; i <= high; i++)
      {
         if(indexLeft > middle)
         {
            copyBuffer[i] = array[indexRight];
            indexRight++;
         }

         else if(indexRight > high)
         {
            copyBuffer[i] = array[indexLeft];
            indexLeft++;
         }
   ...
   public static void merge(Comparable[] array, Comparable[] copyBuffer,int low, int middle, int high)
   {
      ...
      // Copy the just-sorted items between low and high from the copyBuffer back to the array.
      for (int i = low; i <= high; i++) {
         array[i] = copyBuffer[i];
      }
...
class SelectionLowHigh
{
   ...
   {
      int max = low;
      for (int i = low + 1; i < upper; i++) { // Use < instead of <=
